...
|---|---|---|---|---|
| C0 | Put the repo in git (private GitHub), `.gitignore` for `weights/ models/*.onnx models/*.engine data/raw data/test_frames data/calib_frames results/*.csv results/*.npz venv/`. Large files on a shared Drive folder mirroring repo paths. Branches `pose/...` and `lm/...`, PRs to `main` | Atisri | ✅ local repo, commit 375ddfc; GitHub remote ⬜ | everything |
...
| C8 | Adaptation-training harness: `unisign/train_adapt.py` with mT5 frozen, pose stack + projection trainable (5.35 M params), checkpoint-resume, sharded pose data on local disk. Needs OpenASL **train** poses (97 K files, redo the 32 GB archive extraction for train). Used by both tracks for input-distribution shifts | Atisri builds; runs for pose shifts are triggered by the pose owner | ✅ built + Mac-smoked (300 clips, no shift, round-trips through eval); Colab-scale runs pending | P8, P9, P10, L9 |
...
| L3 | Baseline LM cost, off-board: params, checkpoint MB, peak memory, encoder ms / decoder ms per sentence on a T4 for greedy and beam-4, tokens generated per sentence. Also `max_new_tokens` sweep (100 → 64 → 48) vs BLEU: the cap is a free energy lever | L1 | ✅ Mac CPU timing table (RESULTS.md L3); cap sweep: 64 free, 48 −0.14 (L3.2) |
...
| L7 | Encoder input length: with temporal subsampling (P8) the encoder sequence shrinks; measure encoder ms/energy vs T at 30/24/16/12/8 fps-equivalent lengths. One knob, two payoffs | L3 | ✅ off-board: 24 fps −0.17 [−0.61,+0.26], 16 fps −1.61, 12 fps −3.26, 8 fps −8.02 (L7.1); board encoder ms via J-request |
...
| L9 | Weight-only INT8 for mT5 (W8A16) in whichever runtime survives L8; BLEU + memory + power. Activation quant only if W8A16 is clean | L8 | 🔄 Mac half: 571→293 MB, −0.09 BLEU-4 [−0.34,+0.14] (L9.1); board memory/power via J-request |
...
| J5 (= L9 board half) | W8A16 checkpoint on the board, both load modes, with power | copy `weights/openasl_pose_only_slt_pruned_w8.pth` (293 MB) and the updated `unisign/`; `python3 -m unisign.unisign_infer --pkl data/openasl_ref_pose/Ads-4j06eJY-00:07:37.233-00:07:47.200.pkl --ckpt weights/openasl_pose_only_slt_pruned_w8.pth --mt5 weights/mt5-base --device cuda --dtype fp16 --repeat 3 --w8-runtime dequant --power-json results/l9_jetson_w8_dequant_power.json --out results/l9_jetson_w8_dequant.json`, then the same with `--w8-runtime int8` → `l9_jetson_w8_int8*.json` | the JSONs + printed `[infer]` lines; **peak GPU mem** is the number that matters (dequant = fp16 model, int8 = int8 weights in RAM) |
| J3 | Same two runs wrapped in the power logger (once J2 works) | add `--power-json results/l4_jetson_pruned_power.json --power-csv results/l4_jetson_pruned_power.csv --idle-seconds 3` to the J2 and J4 commands (wrapper done, tested with a fake sensor on the Mac) | power JSONs: `avg_watts`, `J_per_sentence`, `mJ_per_token` |
