...
| # | Who | Step | Produces / done when |
|---|---|---|---|
| 3.1 | Atisri | **L3 remainder**: `max_new_tokens` sweep 100 → 64 → 48 on the 976 clips (pruned, beam 4) with CIs | `results/eval_test_pruned_mnt{64,48}_mac.json`; row |
| 3.2 | Atisri | **L9 weight-only INT8 (W8A16)** on the pruned mT5, in whichever runtime survived L8 (TensorRT engines with INT8 weights, or PyTorch dynamic quant as fallback). BLEU on 976 clips with CI, checkpoint MB, then board memory + power via a J-request | `weights/…_w8a16.*`, `results/eval_test_w8a16_mac.json`; row |
...
| 3.4 | Atisri | **L7 encoder length**: encoder ms / energy vs T at 256 / 205 / 137 / 103 / 68 frames (30 / 24 / 16 / 12 / 8 fps-equivalent). Off-board timing first, board via request | `results/l7_encoder_length.json`; row |
...
| # | Who | Step | Produces / done when |
|---|---|---|---|
| 4.1 | Atisri | **Train poses**: `python data/openasl_pose_fetch.py --split train --labels-dir data/openasl_labels --out data/openasl_train_pose` (~97 K files, ~30 GB; resumable). Store on the 4×24 GB rig or Drive, not the Mac | `data/openasl_train_pose/` complete count = 96,477 |
| 4.2 | Atisri | **C8 harness** `unisign/train_adapt.py`: mT5 frozen, pose stack + `pose_proj` + `part_para` trainable (~4.5 M params), AdamW, checkpoint-resume, sharded pose loading. Smoke: 1 epoch on 1 K clips reproduces ≥ 20 BLEU on the 976 test clips | script + `results/c8_smoke.json` |
